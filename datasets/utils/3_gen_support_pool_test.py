...
def main():
    ...
    support_path = os.path.join(root_path, 'support_test')
    ...
    for dataType in ['images']: #, 'train2017']:
        set_crop_base_path = join(support_path, dataType)
        set_img_base_path = join(dataDir, dataType)

        #Cambiar esto. Poner aqui el .json generado a partir del script genera_soporte_nshot
        # (propio)
        annFile = os.path.join(root_path, './final_split_5_shot.json')
